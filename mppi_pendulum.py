...
class MPPI():
    """ MMPI according to algorithm 2 in Williams et al., 2017 'Information Theoretic MPC for Model-Based Reinforcement Learning' """
    def __init__(self, env, K, T, U, lambda_=1.0, noise_mu=0, noise_sigma=1, u_init=1, noise_gaussian=True, downward_start=False):
        self.K = K  # N_SAMPLES
        self.T = T  # TIMESTEPS
        ...
        self.noise = np.random.normal(loc=self.noise_mu, scale=self.noise_sigma, size=(self.K, self.T)) if noise_gaussian else np.full(shape=(self.K, self.T), fill_value=0.9)
        ...
        for _ in range(iter):
            for k in range(self.K):
                ...
                for t in range(self.T):
                    ...
                    s, reward, _, _ = self.env.step([perturbed_action_t])
