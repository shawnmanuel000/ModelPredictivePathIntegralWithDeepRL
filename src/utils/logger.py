...
class Logger():
	def __init__(self, envs, agent, config, force_git=False, **kwconfig):
		...
		self.run_num = len([n for n in os.listdir(log_folder)])
	...
	def log(self, string, stats, debug=True):
		with open(self.log_path, "a+") as f:
			if self.log_num == 0: 
				...
				f.write("\n")
			stats_string = ', '.join([f"'{k}': {self.sci_form(v)}" for k,v in stats.items()])
			f.write(f"{string} <{self.get_time()}> ({{{stats_string}}})\n")
	...
	def get_git_info(self, force_git):
		...
		if force_git: assert len(git_status)==0, "Uncommitted changes need to be committed to log current codebase state"
