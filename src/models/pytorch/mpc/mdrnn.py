...
class MDRNN(torch.nn.Module):
	def __init__(self, state_size, action_size, hidden_size=HIDDEN_SIZE, n_gauss=N_GAUSS, load="", gpu=True):
		super().__init__()
		...
		self.lstm = torch.nn.LSTM(action_size[-1] + state_size, hidden_size).to(self.device)
		self.gmm = torch.nn.Linear(hidden_size, (2*state_size+1)*n_gauss + 2).to(self.device)
	...
	def get_loss(self, latents, actions, next_latents, rewards, dones):
		...
		return (gmm + mse + bce) / (self.state_size + 2)
...
class MDRNNCell(torch.nn.Module):
	def __init__(self, state_size, action_size, hidden_size=HIDDEN_SIZE, n_gauss=N_GAUSS, load="", gpu=True):
		...
		self.device = torch.device('cuda' if gpu and torch.cuda.is_available() else 'cpu')
		self.lstm = torch.nn.LSTMCell(action_size[-1] + state_size, hidden_size).to(self.device)
		self.gmm = torch.nn.Linear(hidden_size, (2*state_size+1)*n_gauss + 2).to(self.device)
		if load: self.load_model(load)
