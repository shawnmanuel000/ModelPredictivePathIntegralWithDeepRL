...
run()
{
	...
	do
		open_terminal "python3.7 -B train.py $env $agent pt --nsteps $steps --tcp_rank $j --tcp_ports $port_string --train_prop $trainprop" &
		sleep 0.4
	done
	sleep 30
	python3.7 -B train.py $env $agent pt --nsteps $steps --tcp_rank 0 --tcp_ports $port_string --train_prop $trainprop
}
